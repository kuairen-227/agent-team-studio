...
## 禁止事項
- Web 検索や外部 URL へのアクセスを試みない（参考情報は事前提供のテキストのみ）。既知の一次情報源を出典として「記載」することは可だが、アクセス・取得はしない
- 事実と推測を混在させない
  ...
  "findings": [
    {
      ...
      "sources": [
        {
          "origin": "<knowledge_base | reference | estimated | unknown のいずれか>",
          "detail": "<出典の手がかり（URL / 文献名 / 見出し / 抜粋。任意）>"
        }
      ],
      "notes": "<補足（任意）>"
    }
...
- \`sources\` は各 finding の出典配列。\`origin\` は \`knowledge_base\` / \`reference\` / \`estimated\` / \`unknown\` の英語値のみ使用する
`;
...
3. 各セル・各全体所見に出典 \`sources\` を付す。Investigation Agent の各 finding が持つ \`sources\` を該当セルへ集約・転記し、全体所見にはその根拠となった出典をまとめる。新たな出典を捏造しない。出典が特定できない場合は \`origin: "unknown"\`、推定は \`origin: "estimated"\` を用い、推測を事実として提示しない
...
- \`cells[].sources\` / \`overall_insights[].sources\` は出典配列。\`origin\` は \`knowledge_base\` / \`reference\` / \`estimated\` / \`unknown\` の英語値のみ。Investigation Agent の \`sources\` を転記・集約する
