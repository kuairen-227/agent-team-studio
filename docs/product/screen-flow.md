...
前提は [ADR-0005 MVP スコープ](../adr/0005-mvp-scope.md) と [user-stories.md](./user-stories.md)。URL ルート等の技術詳細は本ドキュメントの範囲外（[design/](../design/README.md) 側で定義）。
...
| 4 | 結果 | 全エージェント完了後の統合レポートを観点×競合のマトリクス形式で表示し、Markdown エクスポートを提供する | [US-4](./user-stories.md#us-4-統合結果を閲覧しエクスポートする) |
...
- **ナビゲーション**: テンプレート一覧と履歴一覧は常時アクセス可能（グローバルナビ想定）
- **失敗時の取り扱い**:
  - 実行開始失敗（US-2）: 入力フォームに留まりエラー表示
  - エージェント失敗（US-3）: 進捗画面で失敗メッセージを表示。統合失敗時も結果画面で個別結果を表示（US-4）
- **リロード挙動**: 実行中のページリロード時の状態復元は MVP スコープ外（user-stories.md Non-goals）。完了実行の結果画面は再訪可能

## 画面 × 受け入れ基準マッピング

各画面が満たす [user-stories.md](./user-stories.md) の受け入れ基準。
