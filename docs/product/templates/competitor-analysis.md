...
| 参考情報が 10,000 文字超 | 超過分の入力をブロックし、残文字数をカウンタ表示 |
...
5. 各調査結果に出典を付し、由来区分（Web 検索由来 / 知識ベース由来 / 参考テキスト由来 / 推定 / 不明）と該当箇所（Web 検索由来は実在の URL、その他は URL / 文献名 / 見出し / 抜粋）を示す。Web 検索由来の URL は検索結果に記載されたものをそのまま用い、改変・捏造しない。出典が不明・推定の場合はその旨を明示し、推測を事実として提示しない
6. 最終出力は指定された JSON フォーマットに厳密に従う

## 禁止事項
- 「Web 検索結果」に無い URL を出力しない（実在しない出典を捏造しない）。検索結果が「出典取得不可」の企業は知識ベース由来 / 推定で扱い、URL を作り出さない
- 事実と推測を混在させない
...
- `findings`: 競合企業ごとの調査結果配列（企業名・要点リスト・根拠レベル・出典・補足）
...
3. 各セル・各全体所見に出典を付す。Investigation Agent の各調査結果の出典を該当セルへ集約・転記し（Web 検索由来の実在 URL はそのまま保持する）、全体所見にはその根拠となった出典をまとめる。出典・URL を捏造せず、不明・推定はその旨を明示する
...
- 各セル・各全体所見に出典を併記する（由来区分と該当箇所。不明・推定はその旨を明示）

### 2. 内部 JSON
以下の構造で出力する。

- `matrix`: 観点ごとのセル配列（観点キー・競合企業名・要約・根拠レベル・出典）
- `overall_insights`: 観点横断の全体所見（所見テキストと出典）
- `missing`: 欠落した観点とその理由
...
| --- | --- | --- | --- |
| 戦略 | ...（出典: Web検索 / https://example.com/news） | ...（出典: 参考情報 / 見出し『戦略』） | ...（出典: 不明） |
| 製品 | ... | ... | ... |
| 投資 | ... | ... | ... |
| パートナーシップ | ... | ... | 情報不足 |
...
各セル・全体所見の出典は、由来区分（Web 検索由来 / 知識ベース由来 / 参考テキスト由来 / 推定 / 不明）と該当箇所（Web 検索由来は実在の出典 URL）を確度ラベルと対で示す。出典が無い場合は「出典: 不明」と明示し、推測がそのまま事実として表示されないようにする。
...
| Integration Agent が失敗 | `integration:matrix` を `failed` 表示し、原因メッセージを併記 | 結果画面で、統合レポートの代わりに個別 Investigation Agent の出力を閲覧可能にする |
