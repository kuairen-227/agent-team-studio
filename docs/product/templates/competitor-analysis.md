...
5. 各調査結果に出典を付し、由来区分（知識ベース由来 / 参考テキスト由来 / 推定 / 不明）と該当箇所（URL / 文献名 / 見出し / 抜粋）を示す。出典が不明・推定の場合はその旨を明示し、推測を事実として提示しない
...
3. 各セル・各全体所見に出典を付す。Investigation Agent の各調査結果の出典を該当セルへ集約・転記し、全体所見にはその根拠となった出典をまとめる。出典を捏造せず、不明・推定はその旨を明示する
...
| --- | --- | --- | --- |
| 戦略 | ...（出典: 知識ベース / 2024 年公式発表） | ...（出典: 参考情報 / 見出し『戦略』） | ...（出典: 不明） |
| 製品 | ... | ... | ... |
| 投資 | ... | ... | ... |
| パートナーシップ | ... | ... | 情報不足 |
...
各セル・全体所見の出典は、由来区分（知識ベース由来 / 参考テキスト由来 / 推定 / 不明）と該当箇所を確度ラベルと対で示す。出典が無い場合は「出典: 不明」と明示し、推測がそのまま事実として表示されないようにする。
