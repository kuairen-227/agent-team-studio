...
>
> **副産物として発見したバグ**: 統合エージェントの max_tokens=3000 は 3 社以上の場合に出力が切れ出力解析エラーになる。8000 へ修正済み（seed ファイル + `bun run db:seed` は冪等だが既存テンプレートを上書きしないため、今回は `UPDATE templates SET definition = jsonb_set(definition, '{llm,max_tokens_by_role,integration}', '8000') WHERE name = '競合調査'` で DB を直接更新した）。
