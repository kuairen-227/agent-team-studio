...
- ガードフックは `.claude/hooks/guard-secret-access.test.sh`（CI の test ジョブで実行）の 28 ケース（ブロック 20 / 許可 8）で誤検知なしを確認している。二重拡張子偽装（`.env.example.local`）・サフィックス偽装（`.env.example` + 任意文字列）・`env -0` / `env 2>` のダンプ・`node`/`python` バイパス・`process.env.*` アクセス・`find -name .env*` の回帰を含む。正規表現変更時はこのテストが回帰を検出する。
