...
| B | deny glob 拡張 ＋ PreToolUse(Bash) ガードフック | **採用** — deny がカバーしない `source` / `printenv` / `bun -e` 等の経路を、コマンド文字列の決定論的検査でブロックする（[ADR-0007](./0007-ai-driven-dev-architecture.md) の hooks による enforcement と同機構） |
