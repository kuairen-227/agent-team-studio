...
| # | 選択肢 | 判定 |
| - | --- | --- |
| A | 維持（worktree 単位の上書き用） | 却下 — ルート `.env` で同様の上書きが可能。worktree のディレクトリは独立しているため、各 worktree が自分のルート `.env` を持つ形で十分機能する |
...
- ルート `.env`（`.env.example` をコピー）に以下を置く
  ...
  - 必要に応じて API 系: `ANTHROPIC_API_KEY` 等（コメント形式で例示）
...
- compose 内の variable interpolation には fallback（`${VAR:-default}`）を **付けない**。`.env` を真実の場所として一本化するため、`.env` が無いまま起動を試みた場合は compose の未定義変数警告 → DB 起動失敗で fail fast する設計。デフォルト値は `.env.example` のみが持ち、二重管理を避ける
