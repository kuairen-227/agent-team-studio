...
- `storage.googleapis.com` は広域共有ドメインで、IP ベース許可のため同 IP レンジの他バケットへの egress も開く。allowlist 最小限の原則からは広いが、Playwright 本体 zip のリダイレクト先として必要なため受容する。CDN の IP ローテートで許可先が REJECT され得る制約は iptables 版と同様（緩和: `sudo bash .devcontainer/init-firewall.sh` 再適用）。
