...
- 作成日: 2026-06-07
- 関連: ADR-0002（学習プロジェクトの状況設定）, ADR-0033（構造化ロギング・補完関係）, Issue #237（選定と導入）, 親トラッカ #235（開発フロー基盤）
...
- **外部 SaaS へのデータ送信** — エラーイベントが `sentry.io` に送られる。→ 対策: before-send で PII/機密を redact（既存 Pino redact 対象と整合）。学習プロジェクトで扱うデータも実データではない。
- **②基盤運用は学べない** — 監視スタックの自前運用（取り込み基盤・スケール・アップグレード）の経験は得られない。→ 評価: 学習主眼は ①利用運用 と明示的に定めた判断であり、②が必要になれば DSN を GlitchTip 等へ差し替えて別途学べる。
- **free tier の枠** — 5K events/月・retention 約 30 日・performance 少量枠。→ 評価: 学習用途では十分。超過時はサンプリングやレート制御で対応可能。
- **外部アカウント・DSN 管理** — Sentry アカウントと DSN シークレットの管理が増える。→ 対策: DSN は env 部品変数方式（ADR-0028）で注入、未設定時は送信無効化。
