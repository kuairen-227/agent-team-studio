...
- 作成日: 2026-04-21
- 更新日: 2026-04-29（ADR-0016 で導入された DB 隔離モード・ポート割当との接続ルールを追記）
- 関連: Issue #66, ADR-0011（関連）, ADR-0016（後続：DevContainer 統合構成）
...
- ネイティブ worktree（`claude -w`）はベースの DevContainer 内で起動するため、DB やポート等の共有資源を main と共有する。破壊的スキーマ変更や並行 E2E ではこの共有が問題になるため、ADR-0016 で導入した DB 隔離モード・ポート割当と組み合わせて手動 `git worktree add` + 別 DevContainer を選択する運用とする
- 用途別の使い分けルール（DB 改修の規模 / Playwright のデバッグ要否 / 並行性の必要性）は `docs/guides/worktree.md` のユースケースマトリクスで定義する
