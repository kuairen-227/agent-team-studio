...
Claude Code v2.1.49 以降でネイティブ Worktree サポート（`claude -w`）が実装された。当環境は v2.1.89 で対応済み。
...
- ネイティブ worktree はリポジトリルートに `.claude/worktrees/` を生成するため、`.gitignore` への追加が必要（ADR-0009 リポジトリ構成への影響）
