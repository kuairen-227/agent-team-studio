...
- 作成日: 2026-05-02
- 関連: ADR-0008（前提・補足）, ADR-0009（前提）, Issue #85, Issue #76
...
| D | `@anthropic-ai/claude-agent-sdk`（Anthropic 高レベル Agent SDK） | 却下 — agent loop / tool execution / context management を SDK 側に委譲する高レベル抽象。本 MVP はこれらを `packages/agent-core` の `engine.ts`（チーム実行制御）と `agent.ts`（個別エージェント実行）で自前実装する方針（ADR-0009）であり、SDK が提供する抽象と機能が重複する。学習目的（ADR-0002）にも自前実装の方が適う |
...
4. **Interface 抽出は保留** — `LlmProvider` interface 等の抽象化は、2 つ目の provider 採用が現実化した時点で抽出する（Rule of Three）。その時点で本 ADR の方針 2 を一部 superseded する ADR を切り、本 ADR の Status を `accepted（一部 superseded by XXXX）` 形式に更新する（ADR-0016 と同形式）
...
- Spike（Issue #81）が要求する AbortSignal / streaming / `maxRetries` 内蔵 retry が公式 SDK で網羅されており、`llm-integration.md` の設計前提（`messages.stream()` + `.abort()` + SDK 内蔵 retry 委任）と直接整合する
- Claude 固有機能（thinking / prompt caching / citations / tool use 等）に最速で追従できる
- Anthropic が Bun の親会社（2025-12 買収）でファーストパーティ扱いとなり、実行時互換性のリスクが他 SDK より低い
- 学習目的（ADR-0002）として、Claude API の SSE イベント体系を抽象化越しでなく直接扱える
- 切替容易性方針（Decision §2）により、将来 SDK 差し替えが必要になっても変更範囲が `llm-client.ts` の中身に閉じる

### ネガティブ / リスク

- `@anthropic-ai/sdk` への単一プロバイダロックインが発生する。マルチプロバイダ運用が必要になった時点で対応コストが顕在化する（ただし方針 2 により変更範囲は `llm-client.ts` 内に閉じる）
- `llm-client.ts` の公開 API（戻り値型・例外型）を自前で設計する責任が発生する（Spike #81 の知見をもとに実装着手時に設計）
- 2 つ目の provider が現実化した時点で、Interface 抽出 ADR を切る義務が発生する（本 ADR 方針 4 で明示）
- `LLM_API_KEY` は compose の variable interpolation ではなく app コンテナ内で直接参照するため、compose の `${VAR:?}` による fail fast が使えない。`llm-client.ts` の初期化時に未設定を検出して即エラーとする

### 中立

- 公開 API のドメイン型（`TextDelta` 等）の初期配置は `packages/agent-core` 内とする。WebSocket メッセージ型（`apps/web` 側で参照）と直接接合する必要が出た時点で `packages/shared` への移動を検討する。`agent-core → shared` の依存方向は ADR-0008 / ADR-0009 と整合するため移動コストは小さい
