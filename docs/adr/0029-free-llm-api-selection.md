...
- **到達手段の更新（2026-06-06・Issue #252 / [ADR-0034](./0034-llm-client-ai-sdk.md)）**: 恒久方式が方向2（Vercel AI SDK Core）に確定したことに伴い、無料ルート Groq の到達手段は **LiteLLM ゲートウェイ経由から `@ai-sdk/groq` によるネイティブ到達へ変わる**。サービス選定（Groq Llama 3.3 70B を無料ルートとする本再決定）は不変。コード移行・`free-llm-setup.md` の更新は ADR-0034 の実装 Issue に委ねる。
