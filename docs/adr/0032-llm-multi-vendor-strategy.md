...
accepted（短期方針 = 方向1 を採用。恒久方式は 2026-06-06 に方向2 へ確定 — 下記「## 恒久方式の確定」/ [ADR-0034](./0034-llm-client-ai-sdk.md)）

- 作成日: 2026-06-02
- 関連: ADR-0020（再評価対象・方針4 が本 ADR を予約）, ADR-0029（前提・本 ADR の方式で無料ルートを再決定）, ADR-0034（恒久方式の確定先）, Issue #250, Issue #252（恒久方式の確定）
...
>
> **中長期（恒久方式）は 2026-06-06 に確定（Issue #252）**。下記 Decision は当初の保留時点の記録として残す。確定内容は本 ADR 末尾「## 恒久方式の確定（2026-06-06・Issue #252）」および [ADR-0034](./0034-llm-client-ai-sdk.md) を参照。**結論: 方向2（Vercel AI SDK Core）を採用**。当初想定した「数週間の運用実績を観測してから判断する」データ駆動トリガは 1 人運用の学習プロジェクトでは成立しないため、複数ベンダー利用の現時点（2026-06）ベストプラクティスに基づく質的判断で確定した。
...
## 恒久方式の確定（2026-06-06・Issue #252）

上記 Decision で保留していた恒久方式を **方向2（Vercel AI SDK Core）** に確定した。確定 ADR は [ADR-0034](./0034-llm-client-ai-sdk.md)。

### 確定トリガの見直し

当初の確定トリガ「無料ルートを数週間の実利用で回し、429 発生率・運用負荷・品質安定性を観測した時点」は成立しなかった。実機検証（§7-11）は 2026-06-02 の各候補 1 回ずつのクリーン実行のみで、1 人運用の学習プロジェクト（ADR-0002）では無料ルートを数週間自動運用するシナリオが自然発生しないため、観測窓を待っても確定データは蓄積されない。よって**運用データ待ちではなく、複数ベンダー利用の現時点（2026-06）ベストプラクティスに基づく質的判断**で確定した。

### 確定内容と当初分岐との対応

当初 Decision「中長期（恒久運用）」が示した分岐のうち、**方向2 は本 ADR で「非推奨」としていたが、その根拠（ADR-0020 却下理由C）はクライアント側 `useChat` に関する評価であり、本確定が採るサーバー側 AI SDK Core には当てはまらない**ことが判明した。詳細は ADR-0034 Context「却下理由C の射程訂正」。

- **方向1 恒久化**: 却下。ゲートウェイ常駐の運用責務・障害点 +1 が 1 人運用で重く、継続運用の実態も無い。ゲートウェイは任意のインフラ層へ降格。
- **方向2（AI SDK Core）**: **採用**。2026 ベストプラクティス。公開境界（方針2）は不変で `textStream` が `AsyncIterable<string>` の drop-in。`@ai-sdk/anthropic` はネイティブ `/v1/messages` を使う。
- **方向2′（自前 interface）**: 次点。学習価値は最大だが、低レベル差異の自前保守コストが学習価値を上回ると判断。

### ADR 操作

- ADR-0034 を新規作成（恒久方式の確定 + ADR-0020 Decision §1・§3 を supersede / 方針4 を充足）。
- ADR-0020 Status を `accepted（一部 superseded by 0034）` に更新。
- ADR-0029 に、無料ルート Groq の到達手段がゲートウェイ経由から `@ai-sdk/groq` ネイティブへ変わる旨を追記。
- コード移行・`free-llm-setup.md` 更新は実装 Issue に委譲。
