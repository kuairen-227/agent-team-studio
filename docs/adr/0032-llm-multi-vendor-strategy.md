...
| データプライバシー | ○ 自前ホストで経由先を所有（ただし宛先が Gemini 無料枠なら学習利用は不変） | ○ 直結（学習利用は不変） | ○ 直結（学習利用は不変） |
...
| Claude 固有機能喪失の影響 | ─ 非該当（agent 経路で未使用） | ─ 非該当 | ─ 非該当 |
...
**フェーズ分離を採る（proposed）**。短期と中長期で最適解が割れるため、一括では決めない。
...
**方向2（Vercel AI SDK）は非推奨**。理由は好みではなく、ADR-0020 却下理由C（自前 WebSocket スタックで AI SDK の主価値を活かせない）＋ Claude 固有機能が agent 経路で未行使＝ provider 抽象の対価に見合う固有便益がない、で一貫する。

### ADR-0020 / ADR-0029 の扱い

| 最終的に採る方式 | ADR-0020 への操作 | 新 ADR |
| --- | --- | --- |
| 方向1（短期・恒久とも） | supersede 不要（公開型不変・方針2 のまま） | ADR-0029 を更新する ADR（ゲートウェイ運用ルート決定） |
| 方向2′ | 方針2 を一部 supersede。Status を `accepted（一部 superseded by NNNN）` に更新（ADR-0016 と同形式） | 新 ADR 必須（interface 抽出 + 2 実装） |
| 現状維持 | 操作なし | 不要 |

本 ADR の段階では ADR-0020 / ADR-0029 のステータスは変更しない（proposed のため）。最終確定時に上表に従って更新する。
...
- **恒久方式が未確定のまま残る** — 本 ADR だけでは恒久運用ルートが決まらない。→ 対策: Issue #250 の実機検証完了後に本 ADR を `accepted` 化または supersede し、ADR-0029 を再決定する（後続作業として #250 に紐付け）。
