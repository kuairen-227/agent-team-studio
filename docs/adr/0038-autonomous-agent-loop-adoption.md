...
また本ループは [ADR-0007](./0007-ai-driven-dev-architecture.md) 品質保証 3 層の第 3 層（フィードバックループ）Phase 3 との関係を整理する必要がある（→ Decision §6）。
...
- Phase 1 では専用 Evaluator と人手レビュー（`review` / `qa`）が並存する。両者の優先順序・エスカレーション基準（Evaluator が合格とした成果物を人手が否定した場合の処理・記録先）は Phase 1 Issue で定める。
