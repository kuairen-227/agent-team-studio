...
### Test Pyramid（Service / API 層）

unit / integration / e2e の比率設計の指針[^pyramid]。下層（unit）を厚く、上層（e2e）を薄くする。**Service 層（`apps/api`）のモデル**であり、本プロジェクトの現状:
...
### Testing Trophy（Web 層）

Web 層（`apps/web`、React + TanStack Router/Query）は **Testing Trophy**[^trophy] をモデルとする（[ADR-0036](../adr/0036-web-layer-testing-trophy.md)）。Static → Unit → **Integration（最大層）** → E2E の比率で、ユーザー操作と表示の結合を主層に置く。Service 層の Pyramid とは層の特性差（ロジック中心 vs 結合中心）に応じた使い分けであり、判定軸（§1 振る舞い検証・経路独立性）は共通で変わらない。
...
ツール・推奨パターン・実装移行トリガは [guides/testing.md](../guides/testing.md) を参照する。
...
[^trophy]: Kent C. Dodds "The Testing Trophy and Testing Classifications" (2021, <https://kentcdodds.com/blog/the-testing-trophy-and-testing-classifications>)。フロントエンドで integration 層を最大に置く比率モデル
