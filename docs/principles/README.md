...
| Clean Code / Cognitive Complexity | 横断（コード可読性） | [coding.md](./coding.md) |
...
| ISTQB / TDD・BDD（軽量） | 横断（テスト原則）/ qa（領域固有） | [testing.md](./testing.md), [.claude/agents/qa.md](../../.claude/agents/qa.md), [ADR-0010](../adr/0010-development-workflow.md) |
...
| 型駆動 + 軽量 TDD | 横断（実装フロー） | [implement-feature skill](../../.claude/skills/implement-feature/SKILL.md), [guides/testing.md](../guides/testing.md), [ADR-0010](../adr/0010-development-workflow.md) |
...
| 8 | **過剰相互参照** — ドキュメント間で双方向リンク・セクション精度参照を増殖させ、ハブ（README）経由参照を迂回する（[ADR-0021](../adr/0021-doc-cross-reference-policy.md)） | §3 |

## 主題別ファイル

横断軸の中で独立した分量を持つ主題は別ファイルに分離する（[ADR-0019](../adr/0019-split-principles-by-topic.md)）:

- [testing.md](./testing.md): テスト原則（価値あるテストの判定軸 / 業界標準 / アンチパターン）
- [coding.md](./coding.md): コード可読性原則（命名・関数サイズ・ネスト・コメント・型・複雑度）

## 関連

- [ADR-0017](../adr/0017-design-development-principles.md): 本ドキュメント新設の意思決定
- [ADR-0019](../adr/0019-split-principles-by-topic.md): 主題別分割の運用ルール
