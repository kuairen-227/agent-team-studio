...
スタック: vitest + React Testing Library + MSW + jsdom（DOM 実装の既定。実行速度が課題になれば happy-dom を検討）。純粋ロジック・ユニットの `bun:test` は維持し、コンポーネント・結合のみ vitest に載せる。

モック境界はテスト対象の層で使い分ける（[principles §3 過剰モック](../principles/testing.md) 回避）:

| テスト対象 | 境界の置き方 |
| --- | --- |
| コンポーネント・結合テスト | MSW でネットワーク境界をモック。`fetch` や `lib/api.ts` 自体はモックしない |
| `lib/api.ts` / `lib/sentry.ts` *自体*のユニット | `fetch`・SDK を外部 I/O 境界として stub（現状の `api.test.ts` / `sentry.test.ts`） |

### TanStack Query の推奨パターン

- テストごとに新しい `QueryClient` を生成し `retry: false`・`gcTime: 0`、mutation も `retry: false` にする（リトライ待ちの遅延を防ぎ、テスト間でキャッシュが残って後続が古い値で誤 pass するのを防ぐ＝FIRST の Independent）。再取得を検証するテストでは `staleTime: 0` も付ける
...
function createTestQueryClient() {
  return new QueryClient({
    ...
    // mutations.retry も false（フォーム送信テストのリトライ遅延を防ぐ）
    defaultOptions: {
...
// isSuccess だけで終えずデータ本体まで検証する（principles §1.2 経路独立性）
expect(result.current.data).toEqual([{ id: "t1", name: "テンプレA" }]);
```
...
上記は vitest **導入**のトリガであり、`bun:test` → vitest の**完全移行**とは別軸。完全移行はランナー二重管理のメンテコストが問題化した時点で後続 ADR として判断する（[ADR-0036](../adr/0036-web-layer-testing-trophy.md) Consequences）。
