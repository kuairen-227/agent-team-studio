...
2. **タイプラベル**が自動付与される。必要に応じて優先度ラベルを追加する
...
| ラベル | 用途 |
| --- | --- |
| `status:blocked` | ブロック中（open/closed では表現できない状態） |

通常の進捗状態（未着手・作業中・完了）は Issue の open/closed で十分なため、ラベルは設けない。
...
- マイルストーンはリリース単位で作成する（例: `v1.0 - MVP Implementation`）。バージョン命名は [ADR-0031](../adr/0031-version-naming-convention.md) を参照
