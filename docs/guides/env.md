...
- **新しいシークレットファイルを追加したら**、`.claude/settings.json` の `permissions.deny` と `.claude/hooks/guard-secret-access.sh` の検知対象に含まれるか確認する（`.env` 以外の独自命名は自動ではカバーされない）。ガードの挙動は `.claude/hooks/guard-secret-access.test.sh` で検証する
