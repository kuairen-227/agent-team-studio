...
| ファイル | 用途 | Git 追跡 |
| ---------- | ------ | ---------- |
| `.env.example` | インフラ変数テンプレート（compose 用） | する |
...
ルート `.env` は compose/インフラ変数専用（[ADR-0018](../adr/0018-relocate-compose-and-consolidate-env.md)）。compose は同ディレクトリの `.env` を variable interpolation の解決元として自動読み込みする。アプリ固有変数は各 app の `.env` に分離し、インフラ変数との混在を避ける（[ADR-0028](../adr/0028-split-env-infra-and-app.md)）。
...
- **新しい環境変数を追加したら対応する `.env.example` も更新する**（インフラ変数はルート、アプリ変数は各 app）
