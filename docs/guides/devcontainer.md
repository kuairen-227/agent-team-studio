...
`Dockerfile` でベース image に `/home/node/.claude` を node 所有で事前作成しているのは、`claude-auth` named volume が初回マウント時に root:root で作られないようにするため（Docker は image 内マウント先の所有権を volume にコピーする仕様）。詳細は [ADR-0016](../adr/0016-devcontainer-integration.md) 参照。
