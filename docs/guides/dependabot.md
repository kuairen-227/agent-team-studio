...
> [!CAUTION]
> 本ガイド導入直後に設定すること。未設定の場合、`--auto` は CI 通過待ちとして機能せず、Dependabot patch PR が CI を通さず即時マージされる。設定タスクは Issue #110 で管理。
...
この設定はリポジトリ管理者が GitHub UI から行う必要がある。
...
- `.github/dependabot.yml` の `directories` は glob ではなくワークスペースを明示列挙しているため、新規 package を追加した場合は必ず `directories` にパスを追記する
- 例: `apps/admin` を追加したら `/apps/admin` を列挙に加える
