...
> クライアント版は Debian 標準リポジトリのものを使う。サーバー（Postgres 18）とバージョンが異なる場合があるが、`psql` は跨ぎバージョンで動作するため通常用途では問題ない。
...
>
> `.env` の `POSTGRES_DB` を変更した場合、VSCode 拡張は環境変数を参照できないため、本プロファイルの `database` 値を手動で同期する必要がある。
...
| Drizzle Studio のポートが他プロセスと衝突 | `bun run --cwd packages/db db:studio --port <別ポート>` で変更し、`.devcontainer/devcontainer.json` の `forwardPorts` も合わせて更新 |
