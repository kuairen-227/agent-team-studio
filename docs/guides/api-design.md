...
| --- | --- | --- |
| GET | リソースの取得 | 200 |
| POST | リソー���の作成 | 201（即時完了）/ 202（非同期処理開始） |
...
- **サーバー → クライアント**: エージェントのステータス更新、出力ストリーミング、完���通知
...
2. サーバーが各エージェントの `agent:status` を送信（初期���態）
...
```
packages/shared/src/
├── api-types.ts         # REST API のリクエスト���レスポ��ス型
├── ws-types.ts          # WebSocket メッセージ型
└── domain-types.ts      # ド��インモデルの型��Template, Execution 等）
```
