...
Phase 1 では専用 Evaluator と人手レビュー（[`review`](../../.claude/skills/review/SKILL.md) / [`qa`](../../.claude/agents/qa.md)）が併存する。**人手レビューが最終権限**（human-in-the-loop。ADR-0038 ガードレール）。
...
### 7.4 Phase 2 への Go/No-Go 定量基準

ADR-0038 Decision §5 が必須とする定量基準。**初期ドラフト（記事実測が出発点）**。Phase 1 の実測がこれを満たして初めて Phase 2（Generator）に着手する。
...
| **人手介入頻度** | Evaluator フィードバックだけで次アクションが定まった割合 ≥ 80%（指摘を解釈し直す手戻り ≤ 20%） | フィードバックの具体性 = Generator 自律化（Phase 2）の前提。Phase 1 は Generator が人手のため「フィードバックの具体性」で代理測定 |

いずれかを外したら方針を見直す（ADR-0038 再検討契機）。

### 7.5 計測（コスト / 合格率 / 人手介入頻度）と記録様式

Phase 1 は手動運用のため、1 評価ごとに以下を**対象 PR / Issue のコメント**に残し、Go/No-Go 判断時に集計する。
...
集計値（一致率・偽陰性率・平均コスト・介入頻度）を 7.4 の Go 条件と突き合わせて Phase 2 着手可否を判断する。
...
| [.claude/agents/evaluator.md](../../.claude/agents/evaluator.md) | 専用 Evaluator エージェント（Phase 1 実体。プロンプト + 較正ルーブリック） |
