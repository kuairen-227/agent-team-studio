...
調査結果セル・所見の出典を表す共通型（#226）。確度ラベル（`evidence_level`）と対で提示し、ユーザーの独立検証を支える。コード上の SSoT は `packages/shared/src/domain-types.ts` の `Source` / `SourceOrigin`。

```json
{
  "origin": "knowledge_base" | "reference" | "estimated" | "unknown",
  "detail": "<出典の手がかり（任意）>"
}
```
...
`sources` は optional。LLM が省略した場合、UI は「出典: 不明」へフォールバックし、推測がそのまま事実として流通しないようにする（[product doc](../../product/templates/competitor-analysis.md#出力ユーザー向け-markdown)）。Web 検索を伴う出典の本格化（N2）は別マイルストーン（[ADR-0005](../../adr/0005-mvp-scope.md) §v2 ロードマップ）。
...
`evidence_level` の判定基準は Investigation Agent のプロンプト（[product doc](../../product/templates/competitor-analysis.md#investigation-agent共通ひな型)）で LLM に判定させる。`insufficient` の場合、Integration Agent はそのセルを「情報不足」として扱う。`sources` は各 finding の出典配列（上記 Source 型）。
...
```json
{
  ...
  "overall_insights": [{ "text": "<所見1>", "sources": [{ "origin": "...", "detail": "..." }] }],
  "missing": [
    {
      "perspective": "strategy" | "product" | "investment" | "partnership",
...
- **`sources`**: Investigation Agent の各 finding の `sources` をセルへ集約・転記した値。`overall_insights[].sources` は所見の根拠となった出典をまとめる（捏造は禁止）。optional
