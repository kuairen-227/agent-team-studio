...
`packages/agent-core/src/llm-client.ts` に Anthropic SDK の薄い wrapper を置く。

- **抽象インターフェースは切らない** — MVP では Claude API のみ使用するため、`LlmProvider` interface 等はオーバーエンジニアリング。将来プロバイダを追加する時点で interface を抽出すれば十分
- **依存の局所化** — Anthropic SDK への依存は `packages/agent-core` に閉じ込める。`apps/api` や他のパッケージから直接 SDK を import しない。将来の差し替え時の変更範囲を `agent-core` 内に限定できる
