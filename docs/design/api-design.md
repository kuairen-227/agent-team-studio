...
リアルタイム進捗表示（[ADR-0005 MVP スコープ](../adr/0005-mvp-scope.md) / [user-stories.md US-3](../product/user-stories.md)）の通信契約。agent-core が発行する [`AgentEvent`](./agent-execution.md) を WS 層の `WsMessage` に写像する。
...
- `ws-types.ts` の内容は本ドキュメント §WebSocket §メッセージ型 と厳密に一致させる（実装後はコード側を SSoT とし、本 doc は参照リンクに切り替える）
