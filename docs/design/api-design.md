...
type ApiError =
  ...
  | {
      errorCode: "not_found";
      message: string;
      // id は実装側の prefix 付き識別子（`tpl_xxx` / `exec_xxx` 等）。
      // 厳密化する場合は将来的に branded type（TemplateId / ExecutionId）化を検討。
...
| DB 接続不能等のサーバ内部例外 | REST ハンドラ（DB アクセス時） | REST エラーレスポンス（`internal_error` / 500） |
...
| agent-core 内部例外（当該 agent スコープ、output 書き込み失敗・予期せぬ throw 等。`status` UPDATE 自体の失敗は engine スコープ、[websocket-design.md §内部例外の agent スコープ / engine スコープ境界](./websocket-design.md) 参照） | agent-core（202 受理後の非同期処理） | WebSocket `agent:status status="failed"`（reason: `internal_error`） |
| engine 内部例外（`AgentEvent` 発行不能等） | agent-core（202 受理後の非同期処理） | WebSocket `close(1011, "server_error")` |
...
**MVP 範囲外の脆弱経路（割り切り）**: `POST /api/executions` が 202 を返した直後・クライアントが WS 接続を確立する前に engine が即死（例：`run()` 起動時の DB 例外）した場合、`execution:failed` も `close(1011)` もクライアントに届かない。DB は [agent-execution.md §副作用の順序](./agent-execution.md) により真の状態を保持するため、クライアントは手動リロード後に再接続すれば初期スナップショットで `Execution.status` を取得できる（[websocket-design.md §接続ライフサイクル](./websocket-design.md)）。MVP では本経路の自動復旧は対象外。
