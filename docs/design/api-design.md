...
├── ws-types.ts          # WebSocket メッセージ型（詳細は websocket-design.md）
...
- `ws-types.ts` の内容は [websocket-design.md §メッセージ型](./websocket-design.md) と厳密に一致させる（実装後はコード側を SSoT とし、本 doc は参照リンクに切り替える）
