...
| agent-core 内部例外（当該 agent スコープ、output 書き込み失敗・予期せぬ throw 等。`status` UPDATE 自体の失敗は engine スコープ、[websocket-design.md §内部例外の境界](./websocket-design.md) 参照） | agent-core（202 受理後の非同期処理） | WebSocket `agent:status status="failed"`（reason: `internal_error`） |
