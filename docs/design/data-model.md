...
なお [ADR-0009 アーキテクチャ](../adr/0009-architecture.md) のディレクトリ例は `templates.ts` / `executions.ts` の 2 ファイルのみを示しているが、本 doc で追加した `AgentExecution` / `Result` エンティティの schema ファイル分割方針（`executions.ts` への同居か、独立ファイルか）は MVP 実装 Issue で確定する。
...
stateDiagram-v2
    ...
    running --> failed : 全 Investigation 失敗 / Integration 失敗
    completed --> [*]
    failed --> [*]
```
...
- `failed`: 以下いずれかの状態。
  - 全 Investigation AgentExecution が `failed` で Integration を起動できない
  - Integration AgentExecution が `failed`（§5.3 候補 A 前提。A4 で `partial_failure` を導入する場合はこの扱いが変わりうる）

Investigation AgentExecution が 1 件でも `completed` であれば Integration Agent は残りの成功分で実行される（[llm-integration.md §エージェント単位の障害隔離](./llm-integration.md) 方針）。部分失敗時の `Execution.status` 扱いは §5.3 を参照。
...
type TemplateDefinition = {
  schema_version: "1"; // スキーマ進化に備える。ADR-0014 参照
...
`LlmDefaults` の具体値（`model` / `temperature` / `max_tokens`）の SSoT は [llm-integration.md](./llm-integration.md) であり、シードデータ作成時に本定義に転記する。本 doc および `Template.definition` は値を二重管理しない。
...
Integration AgentExecution の `output` と `Result.structured` は同一内容。前者は「エージェントの実行トレース」、後者は「Execution から参照される成果物」として、それぞれ独立した責務を持つ。重複は許容する（[ADR-0014](../adr/0014-mvp-data-model-design.md) 参照）。`Result.markdown` は同じ内容の Markdown 表現であり、`Result.structured` と同一内容を別表現で保持する（§7.2）。

**部分出力は保存しない**。ストリーミング中のエラー時は `output` 未設定のまま `status = failed` とする（[llm-integration.md](./llm-integration.md) 方針）。

## 9. 主要な設計判断

本 doc の構造（AgentExecution / Result の独立エンティティ化、Template.definition のバージョン管理、Execution.parameters の構造化）に関する設計判断・代案・トレードオフは [ADR-0014 MVP データモデル設計方針](../adr/0014-mvp-data-model-design.md) に記録している。
...
| `AgentExecution.agent_id` の一意性（§3 不変条件）の施行場所（DB UNIQUE 制約 / アプリ層チェック / 両方） | MVP 実装 Issue |
| シードデータ（Template レコード、`LlmDefaults` 具体値の転記）の配置（`packages/db/` 内のシードスクリプトの場所） | MVP 実装 Issue |
| `Result.structured` フィールド名の最終形（ER 図上で型名と同名になる懸念あり。物理スキーマ確定時に再検討） | MVP 実装 Issue |
