...
| Integration のみ失敗 | `failed` | `completed`（個別 AgentExecution.output を結果画面に表示） |
...
テンプレート固有の入力。競合調査テンプレートの場合 [`CompetitorAnalysisParameters`](./templates/competitor-analysis.md#入力パラメータ-json-schema) と一致:
...
Integration Agent の最終成果物。両者は同一内容を別表現で保持する:

- `markdown`: [`docs/product/templates/competitor-analysis.md`](../product/templates/competitor-analysis.md) で定義した Markdown レポート（観点×競合マトリクス＋全体所見）。エクスポート（[US-4](../product/user-stories.md#us-4-統合結果を閲覧しエクスポートする)）はこの属性をそのまま返す
- `structured`: 内部保持用の構造化表現
...
型の詳細制約（`matrix[].perspective` の一意性等）は [`docs/design/templates/competitor-analysis.md §失敗観点の表現ルール`](./templates/competitor-analysis.md#失敗観点の表現ルール) を参照。

## 8. AgentExecution.output の構造

`role` により保持するスキーマが異なる:
...
type IntegrationOutput = CompetitorAnalysisResult; // §7.2 と同型
```
...
- 採用理由:
  ...
  - US-4 の失敗時 UX（Integration 失敗 → 統合結果なし、個別結果を表示）が「Result の不在」として自然に表現できる
