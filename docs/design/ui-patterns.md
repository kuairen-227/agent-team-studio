...
本節の上位には [brand.md](../product/brand.md) のブランド軸（AI を魔法にせず、進捗・限界・失敗を誠実に見せる）がある。以下の 7 原則は、その軸を UI 実装に落とすための判断基準。shadcn/ui の素朴なデフォルトを土台に、優先度は上から降順とし、矛盾する場合は上位を取る。
...
shadcn/ui のデフォルトテーマ・余白・タイポを尊重し、独自トークンの導入は最小限にする（[ADR-0005 ロックイン回避](../adr/0005-mvp-scope.md), [brand.md §3](../product/brand.md)）。
