...
| 引数 | サブコマンド | 用途 |
| ---- | ----------- | --- |
| `status` / 引数なし | 進捗概況 | 進捗・blocked・停滞・直近 closed を 1 つの報告に集約（旧 `risk` を吸収） |
...
- **dispatch エコーを省略しない**: Step 1 のエコーは「想定と違うサブコマンドで動いていないか」をユーザーが即座に検知するための仕組み。省略すると status へのサイレントフォールバックが再発する
