...
| | `disable-model-invocation` | 任意 | `true` で手動起動（`/skill`）のみに限定し、Claude の自動起動を抑止。description も常時コンテキストから外れる。**①または②のいずれかを満たす場合に付与** — ①破壊的操作・副作用（ファイル削除・外部 API 呼び出し・デプロイ等）がありタイミングを人が握るべきスキル、②引数必須の単発判断スキルで自動起動が意味をなさないもの（例: `judge-dev-mode` は②のみ該当） |
