...
- **レビュー粒度の指示**（下記参照）
- **削減・本質志向の指示**: 「追加提案と同じ重みで削減提案・『そもそも必要か』の問い直しを出してください。コメント追加で意図を補うより、命名・構造で意図が伝わる修正を優先してください」を明示する
...
追加提案と同等に「削る・問い直す」提案を重視する。エージェントへの委譲プロンプトに以下を含める:

- **そもそも必要か** を問う: 抽象化レイヤー・引数・分岐・型・依存・ファイル・テストケースが本当に要るか
- **命名・構造で語れないか**: コメントを足す前に、関数名・変数名・分割で意図が伝わるなら不要
- **重複・冗長の統合**: 似た分岐・似たユーティリティ・似た状態の集約
- **dead code / 過剰防御**: 到達しないコード・起こり得ないケースへのガード・不要な try/catch
- **YAGNI 違反**: 将来のための拡張点・フラグ・抽象化（architect.md のアンチパターンと整合）

指摘は「追加してください」だけでなく「削れます／不要です／命名で十分です」を同じ重みで出す。Nit に落とさず Should 以上に上げてよい。
