...
種別に応じて適切なエージェントに委譲する。複数エージェントが必要な場合は、すべての Agent 呼び出しを 1 つの assistant turn 内で同時発行してから手順 4 へ進む。順次呼び出しはターン消費が増えるため避けること。
...
横断軸（判断はユーザーに委ねる・スコープを守る等）は [設計・開発原則](../../../docs/principles/README.md) を参照。

review 固有の注意点:

- **良い点を必ず含める**: 指摘だけでなく、良い判断・良い実装も報告する
- **複数エージェントの重複排除**: 同じ指摘が複数エージェントから出た場合は統合する
- **並列化指示の効果は確率的**: 手順 3 の「同時発行」は LLM の挙動に依存するため、実消費ターンはワークフローログで実測し、`.github/workflows/review.yml` の `--max-turns` 値（現在 15、暫定）を運用しながら見直す
