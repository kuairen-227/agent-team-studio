---
name: judge-dev-mode
description: Issue の内容から開発環境モード（単一 DevContainer / claude -w / git worktree + 別 DevContainer）を判断する。Issue 番号を引数に指定する。
...
| ID | 名称 | 構成 |
| --- | --- | --- |
| 単 | 単一 DevContainer | 1 ウィンドウ・1 コンテナ。worktree 不使用 |
...
| シグナル例 | カテゴリ | 推奨 |
| --- | --- | --- |
| docs / README / ADR / lint / CI 設定 / Issue・PR テンプレート修正 | A 文書系 | 単 |
| 単発バグ修正 / 型エラー / lint 修正 / 小規模リファクタ / フロント文言 / DB 無関係の API 追加 | B 軽量改修 | 単 |
| 並行 spike / PoC / 読取中心の調査 / カラム追加程度の DB 変更 / E2E ログ確認 | C 軽量並行 | w |
| DB 破壊的変更（DROP / ALTER COLUMN / RENAME / data migration / rollback 検証） | D 重量級 | WT |
| Playwright の trace viewer / `--ui` デバッグ / 並行 E2E | D 重量級 | WT |
| 大規模リファクタ（数十ファイル横断） / メジャーバージョンアップ / framework 移行 | D 重量級 | WT |
| ホットフィックス（main 中断不可） / PR レビュー対応 + 別 Issue 並行 / 長時間 AI 委任 | D 重量級 | WT |
