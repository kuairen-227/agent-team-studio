...
- **反復上限**: 同一仕様に対する Generator↔Evaluator の往復は上限回数まで。**NEEDS_WORK が 3 回連続**したら同一成果物への自己修復を打ち切り人手へエスカレーションする（収束失敗の目安）。タスクがモデルの単独能力を超えていると判断したら、スプリント機構の追加要否を含めて人手判断を仰ぐ（足場は必要時のみ ── §8）。
- **コスト/時間バジェット**: トークン・時間の上限に達したら続行せず中断・報告する。
- **中断**: 実行中断は AbortController / `AbortSignal.any`、tool レベルの遮断は `permissions.deny`（安全網 = egress firewall を前提とし、起動時に有効性を確認する ── §8）。
